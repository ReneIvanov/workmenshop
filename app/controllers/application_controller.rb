class ApplicationController < ActionController::Base
  ...
  def unauthorized  #response if user is not authorized fo current action
    respond_to do |format|
      format.html { redirect_to new_user_session_path, status: 302; session[:message_unauthorized] = "You have not rights for this action - please sign in with necessary rights." }
